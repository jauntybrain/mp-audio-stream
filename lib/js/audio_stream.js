(async () => {
  ...
  class Processor extends AudioWorkletProcessor {
    ...
    process(_, outputs, __) {
      ...
      for (let dest = 0; dest < out[0].length; dest++) {
        out[0][dest] = 0;
      }

      for (let dest = 0, source = 0; source < copyLength; source += channels) {
        out[1][dest++] = this.buffer[source];
      }
  ...
  window.AudioStream = {
    ...
    setSinkId: async (deviceId) => {
      if (typeof audioCtx.setSinkId !== 'undefined') {
